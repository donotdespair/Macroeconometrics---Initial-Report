...
In the extended model I augment the system using the 'sum of coefficients' and 'dummy initial observation' prior. The sum of coefficients prior takes the average of the lagged values and adds them to the basic equation. This is because these values are assumed to be a good forecast of future observations.

The 'dummy initial observation' prior adds a single dummy observation such that all values are set equal to the averages of intial conditions, up to a scaling factor.
...
priors.emp = calc.priors(p, X = X.emp, Y = Y.emp)
...
A.posterior:
```{R}
round(apply(posteriors.emp.qld$A.posterior, 1:2, mean),3)
```
B.posterior:
```{R}
round(apply(posteriors.emp.qld$B.posterior, 1:2, mean),3)
```
Sigma.posterior:
```{R}
round(apply(posteriors.emp.qld$Sigma.posterior, 1:2, mean),3)
```
# QLD
```{R}
#IRFs
...
Each of the impulse response functions show how variables react to a shock to Log real GDP. There are four different systems, one for each of the key Australian states.

The goal of this paper is to find how different states react to changes in monetary policy, unfortunately I do not yield any results that are statistically different from zero, other than the effect of a shock to GDP on itself.

Looking at the point estimates, we can see that the greatest impact on unemployment levels is in Victoria while the impact of real GDP on unemployment levels is lowest in New South Wales. However, as stated earlier, the results are not statistically different from zero.  
