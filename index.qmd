...
posteriors = calc.posteriors(S = 1000, X = X, Y= Y, priors)
```
...
posteriors.ext = calc.posteriors(S = 1000, X = XN,Y=YN, priors = priors.ext)
...
A.posterior:
```{R}
apply(posteriors.ext$A.posterior, 1:2, mean)
```
B.posterior:
```{R}
apply(posteriors.ext$B.posterior, 1:2, mean)
```
Sigma.posterior:
```{R}
apply(posteriors.ext$Sigma.posterior, 1:2, mean)
```
...
priors = calc.priors(p = 2,X = X, Y = Y)

posteriors.emp = calc.posteriors(S = 1000, X = X, Y= Y, priors)
...
A.posterior:
```{R}
apply(posteriors.emp$A.posterior, 1:2, mean)
```
B.posterior:
```{R}
apply(posteriors.emp$B.posterior, 1:2, mean)
```
Sigma.posterior:
```{R}
apply(posteriors.emp$Sigma.posterior, 1:2, mean)
```
